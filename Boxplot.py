...
def box_plots(month):
    ...
    df["month"] = df.timestamp_ms.apply(lambda s: datetime.fromtimestamp(s / 1000).month)
    if month != "all":
    ...
    with_dict = dict(zip(names_with, with_numbers))
    without_dict = dict(zip(names_without,without_numbers))
    for p in insuf:
        with_dict.pop(p,None)
        without_dict.pop(p,None)

    pos_with = [pos[i-1] for i in with_numbers]
    pos_without = [pos[i-1] for i in without_numbers]
    neu_with = [neu[i-1] for i in with_numbers]
    neu_without = [neu[i-1] for i in without_numbers]
    neg_with = [neg[i-1] for i in with_numbers]
    neg_without = [neg[i-1] for i in without_numbers]
